                ...
                <div class="flex-1 h-screen bg-white flex flex-col">
                    <div class="h-[80px] border border-b-[#D1D6DF] flex justify-between items-center"  x-data="{
                        ...
                        <div class="cursor-pointer h-[80px] pt-[25px] px-[20px] relative hover:bg-[#d2e3fc]" @click="showItems = !showItems">
                            <img class="h-[30px] inline mr-2" src="{{ asset('images/backend/icon_Defaultavatar_30.png') }}" alt="账户"><span class="inline-block min-w-[44px] text-sm">{{ mb_strlen( session('userInfo')['nickname']) > 6 ? mb_substr( session('userInfo')['nickname'], 0, 6). '...' :  session('userInfo')['nickname'] }}</span>
